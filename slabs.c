 ...
 * accordingly.
 */
void slabs_init(const size_t limit, const double factor, const bool prealloc/* true if large pages enabled*/) {
...
static int slab_rebalance_start(void) {
    ...
    if (slab_rebal.s_clsid < POWER_SMALLEST ||
        ...
        no_go = -2; // error ocurred

    s_cls = &slabclass[slab_rebal.s_clsid];

    if (!grow_slab_list(slab_rebal.d_clsid)) {
        no_go = -1; // grow slab list failed
    }

    if (s_cls->slabs < 2)
        no_go = -3; // rarely hit class
