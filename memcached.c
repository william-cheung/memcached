...
#include <assert.h>
#include <limits.h>
#include <sysexits.h>			//  preferable exit codes for programs
#include <stddef.h>
...
static void maxconns_handler(const int fd, const short which, void *arg) { 
    struct timeval t = {.tv_sec = 0, .tv_usec = 10000};	// 10 ms

    if (fd == -42 || allow_new_conns == false) {
        /* reschedule in 10ms if we need to keep polling */
        evtimer_set(&maxconnsevent, maxconns_handler, 0); // maxconns_handler(-1, 0, 0) is called when the specified time has expired
        event_base_set(main_base, &maxconnsevent);
        evtimer_add(&maxconnsevent, &t);
    } else { // if fd != -42 && allow_new_conns == true
...
static rel_time_t realtime(const time_t exptime) {
    ...
    if (exptime > REALTIME_MAXDELTA) { // then get the time in secs after the process started (exptime : absolute)
        ...
        if (exptime <= process_started) // out-dated exptime
            return (rel_time_t)1;
        return (rel_time_t)(exptime - process_started);
    } else { // exptime : relative 
...
static int add_iov(conn *c, const void *buf, int len) {
    ...
    do {
        m = &c->msglist[c->msgused - 1]; // the last msghdr used
        ...
        if (limit_to_mtu && len + c->msgbytes > UDP_MAX_PAYLOAD_SIZE) {
            ...
            len -= leftover; // len = UDP_MAX_PAYLOAD_SIZE - c->msgbytes
        } else {
            leftover = 0;
        }
                  ...
                  const void *cookie)
{
    ...
    if (c->protocol == binary_prot) {
        ...
        size_t needed = vlen + klen + 10; // "STAT key value \r\n"
        if (!grow_stats_buf(c, needed)) {
            return ;
        }
 ...
 */
void do_accept_new_conns(const bool do_accept) { // called by accept_new_conns() in thread.c
    conn *next;
    ...
    for (next = listen_conn; next; next = next->next) {
        if (do_accept) {
            ...
            update_event(next, 0);
            if (listen(next->sfd, 0) != 0) { // do not accept new connections
                perror("listen");
            }
        }
    }
...
static void drive_machine(conn *c) {
    ...
    while (!stop) {
...
#endif
            if (sfd == -1) {
                if (use_accept4 && errno == ENOSYS) { // function not implemented
                    use_accept4 = 0;
                    continue;
                }
        ...
        case conn_nread:
            ...
            if (c->rbytes > 0) {
                int tocopy = c->rbytes > c->rlbytes ? c->rlbytes : c->rbytes; // min
                if (c->ritem != c->rcurr) {
...
static int server_sockets(int port, enum network_transport transport,
    ...
    if (settings.inter == NULL) {
        ...
        for (char *p = strtok_r(list, ";,", &b);
             p != NULL;
             p = strtok_r(NULL, ";,", &b)) { // reentrant version
            int the_port = port;
            char *s = strchr(p, ':');
            if (s != NULL) {
                *s = '\0';
        ...
        free(list);
        return ret; // 0 : success, 1 : failure 
    }
}
