...
#define CLEAR_LRU(id) (id & ~(3<<6)) // id & 00111111
...
item *do_item_alloc(char *key, const size_t nkey, const int flags,
    ...
    for (i = 0; i < 5; i++) { // 5 tries
        ...
        if (!settings.lru_maintainer_thread) { // no lru_maintainer_thread
			// remove(free) expired or flushed items and move active items to WARM_LRU
...
static void do_item_link_q(item *it) { /* item is the new head */
    item **head, **tail;
    assert((it->it_flags & ITEM_SLABBED) == 0); // not a freed chunk
...
void do_item_update(item *it) {
    MEMCACHED_ITEM_UPDATE(ITEM_key(it), it->nkey, it->nbytes);
    if (it->time < current_time - ITEM_UPDATE_INTERVAL) { // accessed 1 min ago
        assert((it->it_flags & ITEM_SLABBED) == 0);
        ...
        switch (cur_lru) {
            ...
            case WARM_LRU:
                ...
                if (sizes[id] > limit) {
                    ...
                    it = search;
                    removed++; // removed from HOT_LRU / WARM_LRU
                    break;
                } else if ((search->it_flags & ITEM_ACTIVE) != 0) {
                    /* Only allow ACTIVE relinking if we're not too large. */
                    itemstats[id].moves_within_lru++;
                    search->it_flags &= ~ITEM_ACTIVE;
                    do_item_update_nolock(search); // move to head
                    do_item_remove(search); // --refcnt
                    item_trylock_unlock(hold_lock);
                } else {
            ...
            case COLD_LRU:
                ...
                if (do_evict) {
                    ...
                    do_item_unlink_nolock(search, hv); // free the item
                    removed++;
                } else if ((search->it_flags & ITEM_ACTIVE) != 0
    ...
    for (i = POWER_SMALLEST; i < MAX_NUMBER_OF_SLAB_CLASSES; i++) {
        ...
        if (s->run_complete) {
            ...
            if (available_reclaims > low_watermark) {
                last_crawls[i] = 0;
                if (next_crawl_wait[i] > 60) // expected a sooner crawl
 ...
 * more clearly. */
static item *crawler_crawl_q(item *it) { // move it toward the head
    item **head, **tail;
    assert(it->it_flags == 1);
    assert(it->nbytes == 0);
...
static void *item_crawler_thread(void *arg) {
    ...
    while (crawler_count) {
        ...
        for (i = POWER_SMALLEST; i < LARGEST_ID; i++) { // only crawl HOT_LRU ?
            ...
            if (search == NULL ||
                (crawlers[i].remaining && --crawlers[i].remaining < 1)) { // --remaining == 0
                if (settings.verbose > 2)
...
int init_lru_crawler(void) { // init stats, conds, locks
